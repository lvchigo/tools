...
int EraseClassData(vector< ClassInfo >& classList)
{
	int i, nRet = TOK;
	vector< ClassInfo >::iterator itClassList;
	map< int, SDClass >::iterator itPool; 
	...
	for(itClassList = classList.begin();itClassList != classList.end(); itClassList++)
	{
		itPool = g_SDParam.m_classpool.find(itClassList->SubClassID);
		if ( itPool != g_SDParam.m_classpool.end() )
			g_SDParam.m_classpool.erase(itPool);

		//remove the feature files for the specified ClassID
		dir = opendir(g_SDParam.m_strFolder.c_str());
		if (NULL == dir)
		{
			printf("The specified folder %s doesn't exist, or have no access right to the folder\n", g_SDParam.m_strFolder.c_str());
		...
		while ((s_dir = readdir(dir))!=NULL)
		{
			if ( IsSDdatFile(s_dir->d_name) && GetClassIDFromFilePath(s_dir->d_name) == itClassList->SubClassID )
			{
				sprintf(szFile, "%s%s", g_SDParam.m_strFolder.c_str(), s_dir->d_name);
				printf("Deleting %s ... ",szFile);
				if ( remove(szFile) == 0 )
					printf("success!\n");
				else
					printf("failure!!!\n");
...
int Init(const char *path, int iSwapLimit)
{
	if (!path || iSwapLimit <= 0 )
		return TEC_INVALID_PARAM;
...
int SimilarDetect(IplImage* pImage, UInt64 ImageID, int ClassID, int SubClassID, SD_RES *result, int BinSaveFeat)
{
	SD_Global_Feature *pFeat = NULL;
	map< int,SDClass >::iterator itPool;
	map<unsigned int, SDCluster>::iterator itCluster;
	...
	if (!pImage || ClassID < 0 || SubClassID < 0 || BinSaveFeat > 1 || BinSaveFeat < 0 || !result)
		return TEC_INVALID_PARAM;

	if (pImage->nChannels != 3 || pImage->depth != IPL_DEPTH_8U || pImage->width < 16 || pImage->height < 16)
		return TEC_UNSUPPORTED;
	...
	MultiBlock_LayoutExtractor(pImage, pFeat->m_feat);

	code = EncodeFeature(pFeat->m_feat);
	result->sMode = SD_eUnique;
	result->id = ImageID;
	...
	else
	{
		...
		for (it = itCluster->second.m_vecFeat.begin();it != itCluster->second.m_vecFeat.end();it++)
		{
			...
			{
				result->sMode = SD_eSame;
				result->id = it->ImageID;
				break;
			}
		}
		if ( (it == itCluster->second.m_vecFeat.end() ) && ( BinSaveFeat == 1 ) )
		{
			itCluster->second.m_vecFeat.push_back(*pFeat);
			itPool->second.m_lFileCounter++;
		}
	}
	if ( ((itPool->second.m_lFileCounter % g_SDParam.m_iSwapLimit) == 0) && ( BinSaveFeat == 1 ) )
