...
void predict(mxArray *plhs[], const mxArray *prhs[], struct svm_model *model, const int predict_probability)
{
	...
	for(instance_index=0;instance_index<testing_instance_number;instance_index++)
	{
		...
		{
            //printf("feature_number %d nr_class %d\n", feature_number, nr_class);
			for(i=0;i<feature_number;i++)
			{
				x[i].index = i+1;
				x[i].value = ptr_instance[testing_instance_number*i+instance_index];
                //printf("(%d, %f) ", x[i].index, x[i].value);
			}
            //printf("\n");
		 ...
		 int nrhs, const mxArray *prhs[] )
{
	...
	if(mxIsStruct(prhs[2]))
	{
        ...
        //int erms = svm_save_model("/home/tangyuan/Projects/GeekEye/source/application/genn/save_by_matlab_svmpredict.model", model);
        //model = svm_load_model("/home/tangyuan/Projects/GeekEye/source/application/genn/save_by_matlab_svmpredict.model");
